...
```sh
cp .env.example .env.local   # fill in GEMINI_API_KEY (or GROQ_API_KEY)
npm install
npm run dev
```
...
**Site configuration → Environment variables → `GEMINI_API_KEY`** (or `GROQ_API_KEY` if `src/api/llm.ts`
points at Groq), then trigger a new deploy.
